...
class PredictiveRNNLayer(nn.Module):
    ...
    def __init__(self, input_dim, hidden_dim, stimulus_dim, below_dim,
                 initializer_hidden_sizes=(), tau=0.9, noise_std=0.1, decoder_scale=1.0):
        super().__init__()
        if not 0.0 <= tau <= 1.0:
...
class StackedRNNPredictor(nn.Module):
    ...
    The latent sequence handed up to each layer -- and the below-state fed to
    each initializer -- is detached, so every layer's gradients stay **entirely
    ...
    def __init__(self, input_dim, hidden_dims, initializer_hidden_sizes=(64,),
        ...
        for i in range(num_layers):
            layers.append(PredictiveRNNLayer(
                ...
                stimulus_dim=input_dim, below_dim=below_dim,
